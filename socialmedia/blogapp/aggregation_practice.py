from django.db.models import Count, Sum, Avg, Max, Min, F, Q, Subquery, OuterRef, Exists, Prefetch
...
def exercise_6_blogs_ordered_by_likes():
    ...
    for blog in blogs:
        print(f"Blog: {blog.title} | Likes: {blog.like_count} | Comments: {blog.comment_count}")
    return blogs
...
def exercise_12_comments_per_blog():
    result = Comment.objects.values('blog__title').annotate(comment_count=Count('id'))
    for row in result:
...
def exercise_14_search_title_or_content():
    result = Blog.objects.filter(Q(title__icontains='hello') | Q(content__icontains='by'))
    for blog in result:
...
def exercise_16_latest_blog_per_user():
    latest_blog = Blog.objects.filter(
        user=OuterRef('id')
    ).order_by('-publish_date').values('title')[:1]

    users = User.objects.annotate(
        latest_blog_title=Subquery(latest_blog)
    )
    for user in users:
...
def exercise_17_blog_latest_comment_content():
    ...
    blogs = Blog.objects.annotate(
        latest_comment_content=Subquery(latest_comment)
    )
    for blog in blogs:
